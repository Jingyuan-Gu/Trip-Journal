...
export function JournalPreview({template,photos,content,editable=false,selectedTextId,onSelectText,overrides={},story,onTextChange,edits=emptySceneEdits,onEdits}:Props){
 ...
 const drag=useRef<{id:string;kind:'photo'|'resize'|'crop'|'text'|'textResize';startX:number;startY:number;base:SceneEdits;x:number;y:number;width:number;height:number;rotation:number;cropX:number;cropY:number;overflowX:number;overflowY:number}|null>(null);
 ...
 const begin=(e:PointerEvent<HTMLElement>,id:string,kind:'photo'|'resize'|'crop'|'text'|'textResize')=>{
  if(!editable||!onEdits||e.button!==0)return;e.preventDefault();e.stopPropagation();e.currentTarget.setPointerCapture(e.pointerId);
  const photo=scene.photos.find(p=>p.photo.id===id),text=scene.texts.find(t=>t.id===id);
  const item=kind==='text'||kind==='textResize'?text:photo;if(!item)return;
  onSelectText?.(photo&&kind!=='text'&&kind!=='textResize'?'photo:'+id:id);
 ...
 <span>{cropId?'拖动照片内容调整取景':'拖动照片 · 拖右下角调整大小'}</span>
 ...
 <label>大小<input aria-label="图片大小" type="range" min="60" max="900" value={activePhoto.width} onChange={e=>{const width=Number(e.target.value);patchActive({width,height:width*activePhoto.height/activePhoto.width});}}/></label>
 ...
 style={{position:'absolute',left:slot.x,top:slot.y,width:slot.width,height:slot.height,padding:slot.padding,paddingBottom:slot.bottom,background:slot.frame,transform:'rotate('+slot.rotation+'deg)',borderRadius:slot.borderRadius}}
 ...
 {measured.map(({t,display})=><div key={t.id} className="text-element" style={{position:'absolute',left:t.x,top:t.y,width:t.width,height:t.lineHeight*t.maxLines,zIndex:4}}>
 {editable&&selectedTextId===t.id&&<button className="text-move" aria-label={'移动'+t.label} onPointerDown={e=>begin(e,t.id,'text')} onPointerMove={move} onPointerUp={finish} onPointerCancel={finish} onClick={e=>e.stopPropagation()}>⠿</button>}
 {editable&&selectedTextId===t.id?<textarea autoFocus aria-label={t.label} className="scene-inline is-text-selected" style={{width:'100%',height:'100%',fontFamily:t.fontFamily,fontSize:t.fontSize,fontWeight:t.fontWeight,color:t.color,lineHeight:t.lineHeight+'px',textAlign:t.align}} value={t.text} maxLength={t.maxLength} onClick={e=>e.stopPropagation()} onChange={e=>onTextChange?.(t.id,e.target.value)} onKeyDown={e=>{if(e.key==='Escape'||((e.ctrlKey||e.metaKey)&&e.key==='Enter')){e.currentTarget.blur();onSelectText?.('');}}}/>:<div role={editable?'button':undefined} tabIndex={editable?0:undefined} aria-label={t.label} className="scene-text" style={{width:'100%',height:'100%',fontFamily:t.fontFamily,fontSize:t.fontSize,fontWeight:t.fontWeight,color:t.color,lineHeight:t.lineHeight+'px',textAlign:t.align,cursor:editable?'text':undefined}} onClick={e=>{if(editable){e.stopPropagation();onSelectText?.(t.id);}}} onKeyDown={e=>{if(editable&&e.key==='Enter')onSelectText?.(t.id);}}>{display|| (editable?'＋ '+t.label:'')}</div>}
 {editable&&selectedTextId===t.id&&<button className="element-resize" aria-label={'调整'+t.label+'宽度'} onPointerDown={e=>begin(e,t.id,'textResize')} onPointerMove={move} onPointerUp={finish} onPointerCancel={finish} onClick={e=>e.stopPropagation()}/>}
 </div>)}
 </div></div></div>;
}
