...
import { buildJournalScene,clamp,ensureDecorationInsideCanvas } from '../services/journalSceneService';
...
export function JournalPreview({template,photos,content,editable=false,selectedTextId,onSelectText,overrides={},story,onTextChange,edits=emptySceneEdits,onEdits}:Props){
 ...
 const move=(e:PointerEvent<HTMLElement>)=>{
  ...
  if(d.kind==='sticker'||d.kind==='stickerResize'){
   ...
   const placed=ensureDecorationInsideCanvas(geometry);if(!placed)return;
   onEdits({...d.base,stickers:{...(d.base.stickers??{}),[d.id]:{...(d.base.stickers??{})[d.id],x:placed.x,y:placed.y,width:placed.width,height:placed.height}}});return;
  }
 ...
 {Boolean(slot.overflow)&&<span className="scene-photo-overflow">+{slot.overflow}</span>}
