...
export default function TemplatePage(){
 ...
 return <PageFrame title="选择手账风格" description="把今天走过的每一站，整理成一页完整的旅行手账。" back="/itinerary" placeholder={false}><div className="template-layout"><section className="paper-panel template-controls"><h2>手账风格</h2><p className="template-prompt-hint">选择风格后，立即预览今天的旅行故事。</p><div className="template-choices">{ids.map(id=>{const t=getTemplateById(id);return <button type="button" key={id} onClick={()=>setState(p=>({...p,selectedTemplateId:id}))} className={`template-choice ${state.selectedTemplateId===id?'chosen':''}`}><span className={`style-swatch ${id}`} /><span><strong>{t.name}</strong><small>{t.description}</small></span><span className="radio-mark" /></button>})}</div><PrimaryButton className="w-full mt-6" onClick={create}>用这个风格生成手账</PrimaryButton></section><section className="preview-surface template-preview-wrap"><span className="preview-label">实时预览 · 今日行程 · {selected.length} 张代表照片 · 3:4</span><JournalPreview template={template} photos={selected} content={state.journalContent} overrides={state.textStyleOverrides} story={story} edits={edits}/></section></div></PageFrame>;
}
