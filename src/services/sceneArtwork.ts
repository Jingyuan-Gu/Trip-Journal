...
export function sceneArtwork(scene:JournalScene):string {
 ...
 const shapes=scene.decorations.map(s=>s.kind==='rect'?(scene.style==='route_journal'?`<path d="M${s.x+3} ${s.y} L${s.x+s.width-5} ${s.y+2} L${s.x+s.width} ${s.y+s.height-5} L${s.x+s.width-12} ${s.y+s.height} L${s.x+4} ${s.y+s.height-2} L${s.x} ${s.y+8}Z" fill="${s.color}" opacity=".94"/>`:scene.style==='soft_scrapbook'?`<rect x="${s.x}" y="${s.y}" width="${s.width}" height="${s.height}" fill="${s.color}" rx="10" opacity=".82"/>`:`<path d="M${s.x} ${s.y+4} L${s.x+s.width} ${s.y} L${s.x+s.width-4} ${s.y+s.height} L${s.x+6} ${s.y+s.height-2}Z" fill="${s.color}"/><path d="M${s.x+10} ${s.y+8}h${Math.max(0,s.width-20)}" stroke="#554b40" opacity=".28"/>`):s.kind==='line'?`<path d="M${s.x} ${s.y}l${s.width} ${s.height}" fill="none" stroke="${s.color}" stroke-width="3"/>`:s.kind==='text'?`<text x="${s.x}" y="${s.y+s.height*.8}" fill="${s.color}" font-size="${s.fontSize??18}" font-family="system-ui" font-weight="700">${s.text??''}</text>`:'').join('');
