...
export async function generateStopCaption(input:{placeName:string;startTime:string;photos:Photo[]}):Promise<string>{
  ...
  const r=await fetch('/api/generate-stop-caption',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({placeName:input.placeName,startTime:input.startTime,images}),signal:AbortSignal.timeout(125000)});
