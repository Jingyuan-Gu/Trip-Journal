...
export async function analyzeDayTrip(photos: Photo[], date: string, onProgress: (message: string) => void, signal?: AbortSignal, existing?: DayItinerary): Promise<DayItinerary> {
  ...
  const priorFor=(stop:typeof stops[number])=>existing?.stops.find(item=>item.manuallyEdited&&sameIds(item.photoIds,stop.photoIds));
  if(mode==='mock'){const result:DayItinerary={date,title:'一天的旅行',summary:'',closingText:'',mode,photoSignature:signature,stops:stops.map(stop=>{const prior=priorFor(stop),sameRepresentatives=prior&&sameIds(prior.representativePhotoIds,stop.representativePhotoIds);return prior?{...stop,placeName:prior.placeName,placeSource:prior.placeSource,placeConfidence:prior.placeConfidence,startTime:prior.startTime,endTime:prior.endTime,representativePhotoIds:prior.representativePhotoIds,caption:sameRepresentatives?prior.caption:stop.caption,captionSource:sameRepresentatives?prior.captionSource:undefined,manuallyEdited:true}:stop;}) as DayItinerary['stops']};itineraryCache.set(`${date}:${signature}`,result);return structuredClone(result);}
  ...
  const final:DayItinerary={...result,stops:stops.map((stop,index)=>{const generated={...result.stops[index],...stop,placeName:result.stops[index].placeName,placeSource:(result.stops[index].locationSource==='gps'?'gps':'ai') as 'gps'|'ai',confidence:result.stops[index].confidence,locationSource:result.stops[index].locationSource,caption:result.stops[index].caption};const prior=priorFor(stop),sameRepresentatives=prior&&sameIds(prior.representativePhotoIds,stop.representativePhotoIds);return prior?{...generated,placeName:prior.placeName,placeSource:prior.placeSource,placeConfidence:prior.placeConfidence,startTime:prior.startTime,endTime:prior.endTime,representativePhotoIds:prior.representativePhotoIds,caption:sameRepresentatives?prior.caption:generated.caption,captionSource:sameRepresentatives?prior.captionSource:generated.captionSource,manuallyEdited:true}:generated;}) as DayItinerary['stops']};
