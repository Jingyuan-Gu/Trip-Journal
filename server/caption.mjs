export async function caption(input) {
  ...
  const body={model:process.env.AI_MODEL||process.env.AGNES_MODEL||'agnes-2.5-flash',messages:[{role:'system',content:prompt},{role:'user',content:[{type:'text',text:JSON.stringify({placeName:String(input.placeName||'未知地点').slice(0,80),startTime:String(input.startTime||'').slice(0,20)})},...images.map(i=>({type:'image_url',image_url:{url:i.dataUrl}}))]}],max_tokens:1024};
  const url=(process.env.AI_BASE_URL||process.env.AGNES_BASE_URL||'https://apihub.agnes-ai.com/v1').replace(/\/+$/,'')+'/chat/completions';
  const r=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${apiKey}`},body:JSON.stringify(body),signal:AbortSignal.timeout(60000),redirect:'error'});
